...
export const ViewStoneType = () => {
  ...
  const { isLoading, isSuccess, refetch, isRefetching, error } = useFetch<StonesTypes[]>({
    endpoint: search === '' 
    ? 'stones/api/v1/stones' 
    : `stones/api/v1/stones?${isRTL ? 'nameAr' : 'nameEn'}[lk]=${search}`,
  ...
  return (
    <>
      ...
      >
        {model ? (
          <CreateStoneType value={editData?.name} item={editData} setDataSource={setDataSource} setShow={setOpen} />
          ) : (
