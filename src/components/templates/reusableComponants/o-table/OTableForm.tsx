...
///
export const OTableForm = ({ editRow, categoriesOptions, karatsOptions, data, setBoxValues, setData, formValues, editData, setEditRow, setEditData }: OTableFormProps_TP) => {
    ...
    const columns: any = [
        ...
        columnHelper.accessor('total_wages', {
            ...
            cell: (info) => (info.row.original.weight * info.row.original.wage).toFixed(3),
        }),
        columnHelper.accessor('wage_tax', {
            header: `${t('wage tax')}`,
            cell: (info) => (info.row.original.weight * info.row.original.wage * .15).toFixed(3),
        }),
    ...
    function deleteRowHandler(id: string) {
        ...
        setBoxValues((prev: OTableDataTypes[]) => prev.filter(row => row.id !== id))
    }
    ...
    //side effects
    useEffect(() => {
        setFieldValue('stock', karatValues.find(item => item.karat === values.karat_value)?.value)
    }, [values.karat_id])
            ...
            <table className='mt-8 border-mainGreen shadow-lg mb-2'>
                    ...
                    {table.getHeaderGroups().map(headerGroup => (
                        <tr key={headerGroup.id} className='' >
                            {headerGroup.headers.map(header => (
                                <th key={header.id} className='p-4 border-l-2 border-l-lightGreen rounded-t-lg'>
                ...
                <tbody className=''>
                    {table.getRowModel().rows.map(row => (
                        <tr key={row.id} className={`${editRow && (editData.id !== row.original.id) ? 'hidden cursor-not-allowed pointer-events-none' : ''}`} >
                            {row.getVisibleCells().map(cell => (
                                <td key={cell.id} className='border-l-2 border-l-flatWhite text-center bg-lightGray'>
                            ...
                            <td className='flex gap-x-2 items-center mt-2'>
                                {
                                    !editRow &&
                                    <DeleteIcon action={() => deleteRowHandler(row.original.id)} />
                                }
                                {
                                    (editRow && editData.id === row.original.id) ?
                                        <button type='submit' className='relative active:top-[1px] py-2 px-2 font-bold rounded-md border-mainGreen border-2' onClick={() => {
                                            setEditRow(false)
                                            const index = data.findIndex((item) => item.id === row.original.id);
                                            const updatedData = [...data];
                                            updatedData[index] = {
                                                ...editData, stock: values.stock || editData.stock, weight: values.weight || editData.weight, wage: values.wage || editData.wage,
                                                karat_value: values.karat_value || editData.karat_value, category_value: values.category_value || editData.category_value
                                            };
                                            setData(updatedData)
                ...
                <tfoot>
                    {
                        editRow ?
                            <tr>
                                ...
                                <td>
                                    <Select
                                        ...
                                        value={{
                                            ...
                                            label: values.category_value || editData.category_value || t('categories'),
                                ...
                                <td>
                                    <Field id='weight' name='weight' type='number' value={values.weight || editData.weight}
                                        onChange={(e: any) => {
                                ...
                                <td>
                                    <Select
                                        ...
                                        value={{
                                            ...
                                            label: values.karat_value || editData.karat_value || t('karats'),
                                ...
                                <td>
                                    <Field id='stock' name='stock' type='number' value={values.stock || editData.stock}
                                        onChange={(e: any) => {
                                ...
                                <td>
                                    <Field id='wage' name='wage' type='number' value={values.wage || editData.wage}
                                        onChange={(e: any) => {
                            ...
                            <tr>
                                ...
                                <td>
                                    <SelectCategory field='id' name='category_id' onChange={(option) => {
                                        setFieldValue('category_value', option!.value)
                                    }}
                                        value={{
                                            value: values.category_value || editData.category_value,
                                ...
                                <td>
                                    ...
                                    <SelectKarat field='id' name="karat_id" onChange={(option) => {
                                        setFieldValue('karat_value', option!.value)
                                        setFieldValue('stock', karatValues.find(item => item.karat === values.karat_value)?.value)
                                    }}
                                        value={{
                                ...
                                <td>
                                    <Field id='total_wages' name='total_wages' value={(Number(values.weight) * Number(values.wage)).toFixed(3)} className="border-none outline-none cursor-default caret-transparent text-center" />
                                </td>
                                <td>
                                ...
                                <td>
                                    {
                                        !editRow &&
                                        <AiOutlinePlus className='cursor-pointer text-lg font-bold bg-mainGray rounded-md w-[30px] h-[30px] shadow-lg active:shadow-none active:w-[28px]' onClick={submitForm} />
                                    }
                                </td>
                            </tr>
